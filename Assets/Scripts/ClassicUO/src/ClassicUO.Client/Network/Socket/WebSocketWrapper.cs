...
// MobileUO: TODO: namespace feature not available in Unity's C#
namespace ClassicUO.Network.Socket
{
    ...
    sealed class WebSocketWrapper : SocketWrapper
    {
        ...
        private async Task ConnectWebSocketAsyncCore(Uri uri)
        {
            ...
            // MobileUO: TODO: SocketsHttpHandler not available in Unity's C#
            throw new NotImplementedException("SocketsHttpHandler not available in Unity's C#");
        ...
        private void GrowReceiveBufferIfNeeded(ref byte[] buffer, ref Memory<byte> memory)
        {
            ...
            if (_rawSocket.Available > MAX_RECEIVE_BUFFER_SIZE)
                // MobileUO: TODO: SocketException with two parameters is not available in Unity's C#
                ...
                throw new Exception($"WebSocket message frame too large: {_rawSocket.Available} > {MAX_RECEIVE_BUFFER_SIZE}");
